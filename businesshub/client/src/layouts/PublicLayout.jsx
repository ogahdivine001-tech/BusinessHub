      ...
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {LINKS.map((l) => (
              <a key={l.to} href={l.to} className="text-sm font-medium text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">
                {l.label}
              </a>
            ))}
          </nav>
        ...
        {open && (
          <div className="md:hidden border-t border-gray-100 dark:border-gray-800 px-4 py-4 space-y-3">
            {LINKS.map((l) => (
              <a key={l.to} href={l.to} onClick={() => setOpen(false)} className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                {l.label}
              </a>
            ))}
      ...
      <footer className="border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="col-span-2 md:col-span-1">
            <Logo />
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-3">Everything your business needs, in one place.</p>
          </div>
          <div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-gray-500 dark:text-gray-400">
              <li><Link to="/privacy" className="hover:text-gray-900 dark:hover:text-white">Privacy</Link></li>
              <li><Link to="/terms" className="hover:text-gray-900 dark:hover:text-white">Terms</Link></li>
            </ul>
          </div>
        </div>
